...
const rateLimitMiddleWare = asyncHandler(async  (req,res,next, data) => {
    ...
    // .user.id
    let userExists = await UserRequest.findOne({ userId: user._id })
    const countDoc = await UserRequest.countDocuments({userId: {$gt : 5}})
    // if(userExists) await userExists.deleteOne()
    if(countDoc) await userExists.deleteOne()
